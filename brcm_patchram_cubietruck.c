...
**						<--tosleep=number of seconds to sleep before
...
void open_device()
{
	if ((uart_fd = open(device_name, O_RDWR | O_NOCTTY)) == -1) {
...
main (int argc, char **argv)
{
	...
	if (uart_fd < 0) {
		fprintf(stderr, "open(%s) failed: %s (%d)", device_name, strerror(errno),
				errno);
		exit(2);
	}
